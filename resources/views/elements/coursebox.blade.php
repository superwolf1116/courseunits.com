    ...
    <div class="col-xs-12 col-md-12 col-lg-12 searchin-bx">
        <div class="card">
            <div class="search-card-img">
                ...
                @include('elements.likeunlikelist', ['cid'=>$allrecord->id])
            </div>
            <div class="card-body">
                <h5 class="card-title"><a href="{{ URL::to( 'course-details/'.$allrecord->slug)}}">{{$allrecord->title}}</a></h5>
                <div class="price-courses">${{$allrecord->price}}</div>
                <p class="card-text">{{$allrecord->title}}</p>
                <h6><span></span> {{$allrecord->User->first_name.' '.$allrecord->User->last_name}}</h6>
                <?php
...
// pass the array to the function
                ...
                <ul class="search-funslity">
                    <li>{{$sum}} total</li>
                    <li>{{$contentCount}} Lectures</li>
                    <li>{{$level[$allrecord->level]}}</li>
